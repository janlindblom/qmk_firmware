...
static int write_code_table_to_memory(buf_cache* pc, SymbolEncoder* se, uint32_t symbol_count) {
    ...
    if (write_cache(pc, &i, sizeof(i))) return 1;

    /* Write the number of bytes that will be encoded. */
    symbol_count = htonl(symbol_count);
    if (write_cache(pc, &symbol_count, sizeof(symbol_count))) return 1;
    ...
    while (count-- > 0) {
        ...
        for (unsigned int curbit = 0; curbit < numbits; ++curbit) {
            ...
            if (get_bit(bytes, curbit)) {
                assert(p != NULL);
                if (p->one == NULL) {
                    p->one = curbit == (unsigned char)(numbits - 1) ? new_leaf_node(symbol) : new_nonleaf_node(0, NULL, NULL);
                    assert(p->one != NULL);
                    p->one->parent = p;
                }
                assert(p->one != NULL);
                ...
                assert(p != NULL);
                if (p->zero == NULL) {
                    p->zero = curbit == (unsigned char)(numbits - 1) ? new_leaf_node(symbol) : new_nonleaf_node(0, NULL, NULL);
                    assert(p->zero != NULL);
                    p->zero->parent = p;
                }
                assert(p->zero != NULL);
...
static huffman_node* read_code_table_from_memory(const unsigned char* bufin, unsigned int bufinlen, unsigned int* pindex, uint32_t* pDataBytes) {
...
int huffman_encode_memory(const unsigned char* bufin, unsigned int bufinlen, unsigned char** pbufout, unsigned int* pbufoutlen) {
    ...
    if (rc == 0) rc = do_memory_encode(&cache, bufin, bufinlen, se);
